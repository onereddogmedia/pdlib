...
{
  t_float g = expf(0.11512925465f * l);
  
  if(g != x->cur_g)
  {
    x->end_g = g;
    x->counter_g = x->ticks;
    x->delta_g = expf(logf(g/x->cur_g)*x->rcp_ticks);
...
static void para_bp2_tilde_ft2(t_para_bp2_tilde *x, t_floatarg q)
{
  ...
  if(a != x->cur_a)
  {
    x->end_a = a;
    x->counter_a = x->ticks;
    x->delta_a = expf(logf(a/x->cur_a)*x->rcp_ticks);
...
static void para_bp2_tilde_ft1(t_para_bp2_tilde *x, t_floatarg f)
{
  ...
  if(f != x->cur_f)
  {
    x->end_f = f;
    x->counter_f = x->ticks;
    x->delta_f = expf(logf(f/x->cur_f)*x->rcp_ticks);
